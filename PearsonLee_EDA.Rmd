...
## Initial Exploration (Vairance and SD) {.smaller}
Overview of how the data for child and parent hights disperse around each respective mean
```{r echo=TRUE}
measures <- c(var(child), sd(child), var(parent), sd(parent))
names(measures) <- c("Chld Var", "Chld SD", "Parent Var", "Parent SD")
measures
# Based on the measures above we can see the dispersion around each respective mean is fairly significant
```
...
```{r echo=FALSE}
# By making a histogram of our data distribution for children heights we can validate that the distribution is normal or not.
...
```{r echo=FALSE}
# By making a histogram of our data distribution for parent heights we can validate that the distribution is normal or not.
# Include input for selection of bin numbers:
inputPanel(
  selectInput("n_breaks", label = "Number of bins:",
              choices = c(10, 20, 35, 50), selected = 20)
...
<ul>
<li>Null Hypothesis: The means of the average heights for the 2 groups are equal (same gender, different gender)</li>
<li>Alternative Hypothesis: The means of the average heights for the 2 groups are <u>not</u> equal</li>
</ul>

## Taking a t.test (Results/Interpretations)
...
## Conclusions and Next Steps
Issues we had listed here...

## References
Picture of references goes here...
